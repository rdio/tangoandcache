...
namespace Rdio.TangoAndCache.Android.UI.Drawables
{
    /// <summary>
    ...
    public class SelfDisposingBitmapDrawable : BitmapDrawable, IByteSizeAware
    {
        ...
        public Uri InCacheKey;

        public bool SupportsBitmapReuse {
            get { return Util.Utils.HasHoneycomb; }
        }
        ...
        protected virtual void OnFreeResources()
        {
            ...
            lock (monitor) {
                if (!SupportsBitmapReuse) {
                    Log.Debug(TAG, "Detected API level that does not support bitmap reuse, recycling bitmap");
                    Bitmap.Recycle();
                }
                Bitmap.Dispose();
