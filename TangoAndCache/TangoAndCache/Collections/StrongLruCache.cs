...
namespace Rdio.TangoAndCache.Collections
{
    ...
    public class StrongLruCache<TKey, TValue> : IDictionary<TKey, TValue>
    {
        ...
        public void Remove(string keyPrefix)
        {
            ...
            Dictionary<TKey, TValue> removedItems = null;

            lock (this.sync) {
                foreach (TKey key in dict.Keys) {
                    if (key.ToString().StartsWith(keyPrefix)) {
                        keysToRemove = keysToRemove ?? new List<TKey>();
                        removedItems = removedItems ?? new Dictionary<TKey, TValue>();
            ...
            if (removedItems != null) {
                foreach (var kvp in removedItems) {
                    OnEntryRemoved(false, kvp.Key, kvp.Value, default(TValue));
                }
            }
        }
